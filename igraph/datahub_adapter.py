...
try:
    ...
    from datahub.sdk import DataHubClient, FilterDsl as SearchFilter
    ...
    async def _live_context(self, entity: str, field: str | None) -> DataHubContext:
        ...
        with self._kit_context(client):
                ...
                try:
                    ...
                    for node in downstream:
                        ...
                        node.glossary_terms = self._names(
                            detail.get("glossaryTerms"),
                            "terms",
                            "term",
                        )
                        node.column_paths = self._names(
                            detail.get("schemaMetadata"),
                            "fields",
                            "fieldPath",
                        )
                except Exception as exc:  # noqa: BLE001
            ...
            if agent_list_schema_fields:
                try:
                    schema_result = agent_list_schema_fields(
                        str(dataset_urn),
                        limit=500,
                    )
